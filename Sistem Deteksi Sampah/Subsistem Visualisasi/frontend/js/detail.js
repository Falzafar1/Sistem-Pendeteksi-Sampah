...
const FRAMES_PER_BATCH = 12; // jumlah frame per load
...
function renderLocationInfo(det) {
  document.getElementById('location-info').innerHTML = `
    ...
    <div class="card-inner">
      ...
      <p class="loc-name">${det.location_name}</p>
      <div class="loc-meta">
        <div class="loc-meta-item">🕐 ${formatTanggal(det.timestamp)}</div>
        <div class="loc-meta-item">🌐 ${det.latitude.toFixed(6)}, ${det.longitude.toFixed(6)}</div>
      </div>
    </div>
  `;
}
...
function renderWasteSummary(det) {
  document.getElementById('count-bottles').textContent = det.plastic_bottle_count;
  document.getElementById('count-cans').textContent    = det.can_count;
  document.getElementById('area-leaf').textContent     = det.leaf_pile_area_m2.toFixed(2);
  document.getElementById('area-mixed').textContent    = det.mixed_waste_area_m2.toFixed(2);
...
function renderFrameBatch() {
  ...
  // Simulasi sedikit delay supaya indicator keliatan
  setTimeout(() => {
    batch.forEach((frame, i) => {
      const nomor  = frameRendered + i + 1;
      const imgSrc = `${API_BASE}/api/frames/image/${frame.image_path}`;
...
async function initPage() {
  try {
    ...
    document.title = `${detection.location_name} — Trash Vision`;
    semuaFrames = frames;
